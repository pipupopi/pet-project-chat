...
import { AUTHORIZATION, ANSWER_REQUEST, REQUEST_HEADERS, REQUEST_METHOD, } from "./const";
import { enterChat } from "./popup";
export async function mailRequest(mail, url) {
    try {
...
export async function changeNameRequest(name, url) {
    try {
        const result = await fetch(url, {
...
export async function userDataRequest(url) {
    try {
        const result = await fetch(url, {
...
export async function messagesRequest(url) {
    try {
        const result = await fetch(url, {
            method: REQUEST_METHOD.GET,
            headers: REQUEST_HEADERS.AUTHORIZATION_HEADERS,
